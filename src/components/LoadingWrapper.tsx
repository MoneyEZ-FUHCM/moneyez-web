...
export function LoadingWrapper({
  ...
  const decoded: any = token ? jwtDecode(token) : CONDITION.NULL;
  const role = decoded
  ...
  useLayoutEffect(() => {
    ...
    if (token && pathname === PATH_NAME.AUTH) {
      router.replace(PATH_NAME.HOME);
    } else if (
      (role === VALID_ROLE.ADMIN || role === CONDITION.NULL) &&
      ...
      router.replace(PATH_NAME.NOT_FOUND);
    } else if (role === VALID_ROLE.USER && pathname !== PATH_NAME.USER) {
      router.replace(PATH_NAME.USER);
    } else if (role === VALID_ROLE.ADMIN && pathname !== PATH_NAME.ADMIN) {
      router.replace(PATH_NAME.ADMIN);
    }
  }, [token, pathname, router, role]);
