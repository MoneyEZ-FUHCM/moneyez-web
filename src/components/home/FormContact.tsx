...
const FormContact = () => {
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    ...
    if (formData.name && formData.email && formData.title && formData.message) {
      showToast(TOAST_STATUS.SUCCESS, "Gửi biểu mẫu thành công", 3);
      setFormData({
        name: "",
        email: "",
        title: "",
      ...
      showToast(TOAST_STATUS.WARNING, "Vui lòng nhập đầy đủ thông tin", 3);
    }
  };

  return (
    <section className="my-36 grid grid-cols-1 items-center justify-center gap-3 lg:grid-cols-3">
      <div className="flex h-[500px] flex-col items-center justify-center p-8 pb-[116px] pl-11 pr-6 pt-[120px] shadow-soft-green lg:col-span-1 xl:pl-[90px] xl:pr-[30px]">
        <div className="space-y-6 text-gray-700">
          {Contacts.map((contact, index) => (
            <div key={index} className="flex items-center space-x-4">
              <div className="rounded-full border border-black p-2 text-lg text-[#4d4d4d] transition-all duration-500 hover:border-primary hover:bg-primary hover:text-superlight">
                {contact.icon}
              </div>
      ...
      <div className="h-[500px] p-8 px-10 py-[50px] shadow-soft-green lg:col-span-2 lg:px-[110px]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="button-hire bg-black">
            GỬI
            <span></span>
            <span></span>
            <span></span>
